...
variable "KIT_DB2_DIR" {
	type		= string
	description = "Path to Db2 LUW 11.5 MP6 FP0 SAP2 Linux on x86_64 64 bit dir for Red Hat 8 and Suse 15. The archive downloaded from SAP Support Portal must be extracted and the path provided to this parameter must contain LABEL.ASC file"
	default		= "/storage/NW75DB2/51055138/DB2_FOR_LUW_11.5_MP6_FP0SAP2_LINUX_"
}

variable "KIT_DB2CLIENT_DIR" {
	type		= string
	description = "Path to DB2 LUW 11.5 MP6 FP0 SAP2 RDBMS Client dir for Red Hat 8 and Suse 15. The archive downloaded from SAP Support Portal must be extracted and the path provided to this parameter must contain LABEL.ASC file."
